...
2. 4 reçete profili × OG sweep matrisi (V18.2 vs V19 top-1 prediction comparison)
...
m_v3 = xgb.Booster(); m_v3.load_model('_v19_model_slug.json')
le_v2 = json.load(open('_v18_2_label_encoder_slug.json', encoding='utf-8'))
le_v3 = json.load(open('_v19_label_encoder_slug.json', encoding='utf-8'))
...
print(f'\n{"feature":<10} {"V18.2 gain":>14} {"V19 gain":>14} {"Δ":>10} {"Δ%":>8}')
...
for pname, base in PROFILES.items():
    print(f'\n--- {pname} ---')
    print(f'{"OG":<8} {"V18.2 top-1":<35} {"V19 top-1":<35} {"match":<7}')
    for og in OG_RANGE:
        b = dict(base)
        b['og'] = og
...
v3_metrics = json.load(open('_v19_metrics.json', encoding='utf-8'))
...
print(f'\n{"slug":<40} {"V18.2 n":>8} {"V18.2 t1":>10} {"V19 n":>8} {"V19 t1":>10} {"Δt1":>8}  {"V18.2 t3":>10} {"V19 t3":>10} {"Δt3":>8}')
for s in WEAK_SLUGS:
    v2 = v2_slug.get(s, {})
    v3 = v3_slug.get(s, {})
    if not v2 or not v3:
...
print(f'V19 slug top-1: {v3_metrics["slug"]["test_top1"]*100:.2f}%')
...
print(f'V19 slug top-3: {v3_metrics["slug"]["test_top3"]*100:.2f}%')
...
print(f'V19 14cat top-1: {v3_metrics["14cat"]["test_top1"]*100:.2f}%')
