// V5 multi-model ensemble inline — KNN + RF (pre-trained) + rule
// Random Forest tek seferde tüm dataset'le eğitilir, model JSON olarak HTML'e gömülür.
const fs = require('fs');

// v1.1: Slug aliases applied — dataset labels already canonical (see style_aliases.json)
const DS = JSON.parse(fs.readFileSync(__dirname + '/_ml_dataset_normalized.json', 'utf8'));
const ALIASES_RAW = JSON.parse(fs.readFileSync(__dirname + '/style_aliases.json', 'utf8'));
delete ALIASES_RAW._meta;
...
(function(){
  ...
  const __ALIASES = JSON.parse(\`${escape(JSON.stringify(ALIASES_RAW))}\`);
...
fs.writeFileSync(__dirname + '/_inline_v5_snippet.html', inlineBlock);
console.log('→ _inline_v5_snippet.html yazildi ('+(inlineBlock.length/1024).toFixed(1)+' KB)');
