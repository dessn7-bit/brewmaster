#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""V17 retrain — V16 + rmwoods (dedup) = 301K reçete.

Ana fark V16'dan:
- DATA_PATH = working/_v17_dataset.json
- bjcp_main_category compute (SLUG_TO_CLUSTER), rmwoods'da yok
- 80/20 stratified split bjcp_slug üzerinden (V17'de 'in_split' yok)
- detect_features compute SKIP — V17 dataset zaten 81-feature ile geldi
"""
...
print(f'  V17 dataset: {len(recipes)} recipes, {len(feat_list)} features', flush=True)
...
# ==================== 14-cat ====================
print('\n' + '=' * 60, flush=True)
print('V17 14-CATEGORY MODEL', flush=True)
print('=' * 60, flush=True)
...
print(f'Training V17 14cat (depth=3, n_est=200, hist)...', flush=True)
...
print(f'\n[V17 14cat] train: {t1_tr:.4f}  test t1: {t1:.4f}  t3: {t3:.4f}  t5: {t5:.4f}', flush=True)
...
m.get_booster().save_model('_v17_model_14cat.json')
sz = os.path.getsize('_v17_model_14cat.json') / 1024
print(f'\nWrote _v17_model_14cat.json ({sz:.0f} KB)', flush=True)
with open('_v17_label_encoder_14cat.json', 'w', encoding='utf-8') as f:
...
# ==================== Slug ====================
print('\n' + '=' * 60, flush=True)
print('V17 SLUG MODEL (≥10 recipes/slug)', flush=True)
print('=' * 60, flush=True)
...
print(f'Training V17 slug (depth=4, n_est=300, hist)...', flush=True)
...
print(f'\n[V17 slug] test t1: {ts1:.4f}  t3: {ts3:.4f}  t5: {ts5:.4f}', flush=True)
...
ms.get_booster().save_model('_v17_model_slug.json')
sz_s = os.path.getsize('_v17_model_slug.json') / 1024
print(f'\nWrote _v17_model_slug.json ({sz_s:.0f} KB)', flush=True)
with open('_v17_label_encoder_slug.json', 'w', encoding='utf-8') as f:
...
with open('_v17_metrics.json', 'w', encoding='utf-8') as f:
    json.dump(out, f, indent=2, ensure_ascii=False)
print('\nWrote _v17_metrics.json', flush=True)
print('[V17 TRAIN DONE]', flush=True)
