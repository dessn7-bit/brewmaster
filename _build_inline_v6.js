// V6.0 multi-model ensemble inline — KNN + RF + rule (Faz 1: Alias normalization)
// Random Forest tek seferde tüm dataset'le eğitilir, model JSON olarak HTML'e gömülür.
const fs = require('fs');

// V6.0: Use alias-normalized dataset (1031 recipes, normalized labels)
const DS = JSON.parse(fs.readFileSync(__dirname + '/_ml_dataset_v6.json', 'utf8'));
...
fs.writeFileSync(__dirname + '/_inline_v6_snippet.html', inlineBlock);
console.log('→ _inline_v6_snippet.html yazildi ('+(inlineBlock.length/1024).toFixed(1)+' KB)');
