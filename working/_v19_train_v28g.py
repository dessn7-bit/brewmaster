...
DATA_PATH = 'working/_v28g_aliased_dataset.json'
...
SLUG_TO_CLUSTER = {
    ...
    # CLUSTER 14: wheat (6 slug + belgian_witbier + Sub-sprint 1 V28f 3 yeni slug)
    ...
    # Sub-sprint 2A V28g Rye Ale reslug ile yeni slug (KAAN KARAR 2: roggenbier -> wheat)
    'roggenbier':'wheat',
    # Sub-sprint 1 V28f Hefeweizen reslug ile yeni slug'lar (BA 2026 destekli)
    'south_german_bernsteinfarbenes_weizen':'wheat',
    'german_leichtes_weizen':'wheat',
    'american_wheat_beer':'wheat',
    # CLUSTER 13'e ekleme: AMERICAN_WHEAT_WINE -> strong_ale (KAAN KARAR 1)
...
m.get_booster().save_model('working/_v19_v28g_model_14cat.json')
sz = os.path.getsize('working/_v19_v28g_model_14cat.json') / 1024
print(f'\nWrote working/_v19_v28g_model_14cat.json ({sz:.0f} KB)', flush=True)
with open('working/_v19_v28g_label_encoder_14cat.json', 'w', encoding='utf-8') as f:
...
print(f'Training V19 slug V28g (Senaryo 5: depth=4, n_est=350, subsample=0.6, colsample=0.6, gamma=0.5, mcw=6, reg_lambda=2.5)...', flush=True)
ms = xgb.XGBClassifier(objective='multi:softprob', num_class=len(le_s.classes_),
    max_depth=4, learning_rate=0.1, n_estimators=350,
    subsample=0.6, colsample_bytree=0.6, gamma=0.5, tree_method='hist',
    eval_metric='mlogloss', random_state=42, verbosity=1,
...
ms.get_booster().save_model('working/_v19_v28g_model_slug.json')
sz_s = os.path.getsize('working/_v19_v28g_model_slug.json') / 1024
print(f'\nWrote working/_v19_v28g_model_slug.json ({sz_s:.0f} KB)', flush=True)
with open('working/_v19_v28g_label_encoder_slug.json', 'w', encoding='utf-8') as f:
