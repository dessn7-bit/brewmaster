"""V11 retrain — V10.1 + 7 new feature engineering features (76 total)
Same hyperparams: depth=3, n_est=200"""
import json, os
import numpy as np
...
print('V11 dataset:', len(recipes), 'recipes,', len(feat_list), 'features')
...
imp = list(zip(feat_list, m.feature_importances_))
imp.sort(key=lambda x: -x[1])
for name, gain in imp[:20]:
    new_marker = ' [NEW]' if name in ['has_coffee','has_fruit','has_spice','has_chili','has_smoke','has_belgian_yeast','has_clean_us05_isolate'] else ''
    print(f'  {name:<32} {gain:.4f}{new_marker}')

# Find where new features rank
print('\nNew feature ranks:')
for new_f in ['has_coffee','has_fruit','has_spice','has_chili','has_smoke','has_belgian_yeast','has_clean_us05_isolate']:
    rank = next((i for i,x in enumerate(imp) if x[0]==new_f), -1)
    importance = imp[rank][1] if rank >= 0 else 0
    print(f'  {new_f:<28} rank #{rank+1}/{len(imp)} importance={importance:.5f}')
...
out = {'cv_mean':cv_mean,'train_top1':t1_tr,'test_top1':t1,'test_top3':t3,'test_top5':t5,'per_class_14cat':per_class,'feature_importances_top20':[(n,float(g)) for n,g in imp[:20]],'sources':data['meta']['sources']}
