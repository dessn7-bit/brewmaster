#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""V16 retrain — V15 + AHA = 9552 reçete.
_v15_train.py'den kopya, sadece DATA_PATH değişti.
"""
...
DATA_PATH = 'brewmaster_v16_dataset.json'

with open(DATA_PATH, 'r', encoding='utf-8') as f:
    data = json.load(f)
recipes = data['recipes']
feat_list = data['meta']['feature_list']
print(f'V16 dataset: {len(recipes)} recipes, {len(feat_list)} features')
...
def detect_features(r):
    ...
    if isinstance(y, list): y = ' '.join(str(x) for x in y)
    yLower = str(y).lower()
...
for r in recipes:
    ...
    for k, v in feats.items(): r['features'][k] = v
...
X_te_kept = X_te[keep]
print(f'Classes: {len(le.classes_)}, train n={len(X_tr)}, test n={len(X_te_kept)}')
...
print(f'\n[V16 14cat] train: {t1_tr:.4f}  test t1: {t1:.4f}  t3: {t3:.4f}  t5: {t5:.4f}')
...
m.get_booster().save_model('_v16_model_14cat.json')
sz = os.path.getsize('_v16_model_14cat.json') / 1024
print(f'\nWrote _v16_model_14cat.json ({sz:.0f} KB)')
with open('_v16_label_encoder_14cat.json', 'w', encoding='utf-8') as f:
    json.dump({'classes': le.classes_.tolist(), 'n_classes': len(le.classes_), 'feature_list': feat_list},
              f, indent=2, ensure_ascii=False)

# ==================== Slug ====================
print('\n' + '=' * 60)
print('V16 SLUG MODEL (>=10 recipes/slug)')
print('=' * 60)
slug_counts = Counter(r['bjcp_slug'] for r in recipes if r.get('bjcp_slug'))
slug_keep = {s for s, c in slug_counts.items() if c >= 10}
print(f'Slugs ≥10 recipe: {len(slug_keep)}')
...
print(f'[V16 slug] test t1: {ts1:.4f}  t3: {ts3:.4f}  t5: {ts5:.4f}')
...
spotlight = ['german_oktoberfest_festbier', 'belgian_gueuze', 'belgian_lambic',
             ...
             'south_german_hefeweizen', 'belgian_witbier', 'south_german_weizenbock',
             'specialty_beer', 'american_amber_red_ale', 'berliner_weisse',
             'oud_bruin', 'smoked_beer', 'fruit_beer']
...
for sl in spotlight:
    if sl in slug_per:
        ...
        print(f'  {sl:<40} (not in test or <10 train)')

ms.get_booster().save_model('_v16_model_slug.json')
sz_s = os.path.getsize('_v16_model_slug.json') / 1024
print(f'\nWrote _v16_model_slug.json ({sz_s:.0f} KB)')
with open('_v16_label_encoder_slug.json', 'w', encoding='utf-8') as f:
...
out = {
    ...
    '14cat': {
        'cv_mean': cv_mean, 'train_top1': t1_tr,
...
with open('_v16_metrics.json', 'w', encoding='utf-8') as f:
    json.dump(out, f, indent=2, ensure_ascii=False)
print('Wrote _v16_metrics.json')
