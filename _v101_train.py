"""V10.1 retrain — XGBoost depth=3 n_est=200, V10 hyperparams"""
import json, os
import numpy as np
...
with open('_ml_dataset_v101_pre_retrain.json', 'r', encoding='utf-8') as f:
    data = json.load(f)
recipes = data['recipes']
feat_list = data['meta']['feature_list']
print('V10.1 dataset:', len(recipes), 'recipes')
...
print(f'\n[V10.1 14cat] train top-1: {t1_tr:.4f}  test top-1: {t1:.4f}  top-3: {t3:.4f}  top-5: {t5:.4f}')
...
# Critical clusters
critical = ['Belgian Strong / Trappist', 'British Bitter / Mild', 'British Strong / Old', 'Specialty / Adjunct', 'Sour / Wild / Brett', 'Belgian Pale / Witbier']
print('\nCritical:')
for cat in critical:
    if cat in le.classes_:
        ci = list(le.classes_).index(cat)
        it = [i for i,y in enumerate(y_te_enc) if y==ci]
        if it:
            t1c = sum(1 for i in it if preds[i]==ci)
            t3c = sum(1 for i in it if ci in proba[i].argsort()[-3:])
            print(f'  {cat}: test_n={len(it)} top-1={t1c}/{len(it)} top-3={t3c}/{len(it)}')

# 73class quick eval (Quadrupel focus)
...
for slug in ['belgian_dubbel','belgian_tripel','belgian_quadrupel','belgian_strong_dark_ale','belgian_strong_golden','belgian_blonde_ale','belgian_witbier','flanders_red_ale','belgian_fruit_lambic','gose']:
    if slug in les.classes_:
        ...
        if it:
            ...
            print(f'  {slug:<30} test_n={len(it)} top-1={t1c} top-3={t3c}')

m.get_booster().save_model('_v101_model_14cat.json')
sz = os.path.getsize('_v101_model_14cat.json')/1024
print(f'\nWrote _v101_model_14cat.json ({sz:.0f} KB)')
with open('_v101_label_encoder_14cat.json','w',encoding='utf-8') as f:
...
with open('_v101_metrics.json','w',encoding='utf-8') as f: json.dump(out, f, indent=2, ensure_ascii=False)
print('Wrote _v101_metrics.json')
