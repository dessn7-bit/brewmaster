// V6.1 multi-model ensemble inline — KNN + RF + rule (Faz 2: Tier 1 expansion)
// Random Forest tek seferde tüm dataset'le eğitilir, model JSON olarak HTML'e gömülür.
const fs = require('fs');

// V6.1: Use Tier 1 expanded dataset (1045 recipes, normalized labels + tier1 expansion)
const DS = JSON.parse(fs.readFileSync(__dirname + '/_ml_dataset_v6_1.json', 'utf8'));
...
fs.writeFileSync(__dirname + '/_inline_v6_1_snippet.html', inlineBlock);
console.log('→ _inline_v6_1_snippet.html yazildi ('+(inlineBlock.length/1024).toFixed(1)+' KB)');
