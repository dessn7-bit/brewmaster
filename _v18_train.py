...
print(f'  V18 dataset: {len(recipes)} recipes, {len(feat_list)} features', flush=True)
...
# ==================== 14-cat ====================
print('\n' + '=' * 60, flush=True)
print('V18 14-CATEGORY MODEL', flush=True)
print('=' * 60, flush=True)
...
print(f'Training V18 14cat (depth=3, n_est=200, hist)...', flush=True)
...
print(f'\n[V18 14cat] train: {t1_tr:.4f}  test t1: {t1:.4f}  t3: {t3:.4f}  t5: {t5:.4f}', flush=True)
...
m.get_booster().save_model('_v18_model_14cat.json')
sz = os.path.getsize('_v18_model_14cat.json') / 1024
print(f'\nWrote _v18_model_14cat.json ({sz:.0f} KB)', flush=True)
with open('_v18_label_encoder_14cat.json', 'w', encoding='utf-8') as f:
...
# ==================== Slug ====================
print('\n' + '=' * 60, flush=True)
print('V18 SLUG MODEL (≥10 recipes/slug)', flush=True)
print('=' * 60, flush=True)
...
print(f'Training V18 slug (depth=4, n_est=300, hist) — KURAL 4 reg tuning: alpha=1.0, lambda=2.0, min_child_weight=5...', flush=True)
...
slug_gap = ts1_tr - ts1
print(f'\n[V18 slug] train t1: {ts1_tr:.4f}  test t1: {ts1:.4f}  t3: {ts3:.4f}  t5: {ts5:.4f}', flush=True)
print(f'[V18 slug] TRAIN-TEST GAP: {slug_gap:.4f} ({slug_gap*100:+.2f}pp) — KURAL 4: <5pp PASS, >=5pp FAIL', flush=True)
if slug_gap < 0.05:
...
ms.get_booster().save_model('_v18_model_slug.json')
sz_s = os.path.getsize('_v18_model_slug.json') / 1024
print(f'\nWrote _v18_model_slug.json ({sz_s:.0f} KB)', flush=True)
with open('_v18_label_encoder_slug.json', 'w', encoding='utf-8') as f:
...
with open('_v18_metrics.json', 'w', encoding='utf-8') as f:
    json.dump(out, f, indent=2, ensure_ascii=False)
print('\nWrote _v18_metrics.json', flush=True)
