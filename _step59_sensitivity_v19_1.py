"""Sprint 56a Sensitivity Test (KURAL 3) — V19 vs V18.3 karşılaştırma.

İki test:
1. 5 stat feature gain karşılaştırma (V19 vs V18.3 XGBoost feature_importances_)
   - 5 stat = og, fg, abv, ibu, srm
   - Hipotez: V19'de ABV outlier bug 5 stat gain'i bastırdı; V18.3 outlier drop sonrası gain artmalı
2. 4 reçete profili × OG sweep matrisi (V19 vs V19.1 top-1 prediction comparison)
...
print('[1] Loading V19 + V18.3 models + label encoders...', flush=True)
m_v2 = xgb.Booster(); m_v2.load_model('_v19_model_slug.json')
m_v3 = xgb.Booster(); m_v3.load_model('_v19_1_model_slug.json')
le_v2 = json.load(open('_v19_label_encoder_slug.json', encoding='utf-8'))
le_v3 = json.load(open('_v19_1_label_encoder_slug.json', encoding='utf-8'))
print(f'  V19 classes={le_v2["n_classes"]}, features={len(le_v2["feature_list"])}')
print(f'  V18.3 classes={le_v3["n_classes"]}, features={len(le_v3["feature_list"])}')


# ── Test 1: Feature gain comparison ──
print('\n=== TEST 1: 5 stat feature gain (V19 vs V18.3) ===', flush=True)
...
print(f'\n{"feature":<10} {"V19 gain":>14} {"V19.1 gain":>14} {"Δ":>10} {"Δ%":>8}')
...
print(f'\n5-STAT TOTAL: V19={stat_pct_v2:.2f}%  V18.3={stat_pct_v3:.2f}%  Δ={stat_pct_v3-stat_pct_v2:+.2f}pp')
...
for pname, base in PROFILES.items():
    print(f'\n--- {pname} ---')
    print(f'{"OG":<8} {"V19 top-1":<35} {"V19.1 top-1":<35} {"match":<7}')
    for og in OG_RANGE:
        b = dict(base)
        b['og'] = og
...
print('\n\n=== TEST 3: Per-slug holdout t1/t3 (V19 vs V18.3) ===', flush=True)
v2_metrics = json.load(open('_v19_metrics.json', encoding='utf-8'))
v3_metrics = json.load(open('_v19_1_metrics.json', encoding='utf-8'))
...
print(f'\n{"slug":<40} {"V19 n":>8} {"V19 t1":>10} {"V19.1 n":>8} {"V19.1 t1":>10} {"Δt1":>8}  {"V19 t3":>10} {"V19.1 t3":>10} {"Δt3":>8}')
for s in WEAK_SLUGS:
    v2 = v2_slug.get(s, {})
    v3 = v3_slug.get(s, {})
    if not v2 or not v3:
...
# Headline metrics
print('\n\n=== HEADLINE METRICS ===')
print(f'V19 slug top-1: {v2_metrics["slug"]["test_top1"]*100:.2f}%')
print(f'V19.1 slug top-1: {v3_metrics["slug"]["test_top1"]*100:.2f}%')
print(f'Δ = {(v3_metrics["slug"]["test_top1"] - v2_metrics["slug"]["test_top1"])*100:+.2f}pp')
print()
print(f'V19 slug top-3: {v2_metrics["slug"]["test_top3"]*100:.2f}%')
print(f'V19.1 slug top-3: {v3_metrics["slug"]["test_top3"]*100:.2f}%')
print(f'Δ = {(v3_metrics["slug"]["test_top3"] - v2_metrics["slug"]["test_top3"])*100:+.2f}pp')
print()
print(f'V19 14cat top-1: {v2_metrics["14cat"]["test_top1"]*100:.2f}%')
print(f'V19.1 14cat top-1: {v3_metrics["14cat"]["test_top1"]*100:.2f}%')
